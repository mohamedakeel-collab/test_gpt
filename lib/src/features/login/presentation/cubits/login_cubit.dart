...
class LoginCubit extends AsyncCubit<LoginEntity> {
  ...
  Future<void> login({required String login, required String password}) async {
    ...
    await result.fold(
          (failure) async {
        emit(AsyncFailure<LoginEntity>(failure));
      },
          (data) async {
        ...
        if (isProvider && user.role != 'hr') {
          emit(AsyncFailure<LoginEntity>(
            PermissionFailure(customMessage: LocaleKeys.accountCannotAccess.tr()),
          ));
          return;
        }

        await _tokenStorage.save(access: data.token);
        await _userCubit.setUserLoggedIn(user: user, token: data.token);

        emit(AsyncSuccess<LoginEntity>(data));
      },
    );
  }
}